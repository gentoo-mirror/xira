...
#Note: there's nothing preventing this from working on stable, but the dependencies of stylize aren't keyworded for
...
pkg_setup() {
	if tc-is-gcc; then
		[[ $(gcc-major-version) -ge 10 ]] && [[ $(gcc-minor-version) -ge 2 ]] && [[ $(gcc-micro-version) -ge 0 ]] \
			&& export ACTIVE_CC="$(gcc-fullversion)" || die
	elif tc-is-clang; then
		[[ $(clang-major-version) -ge 14 ]] \
			&& export ACTIVE_CC="$(clang-fullversion)"|| die
	fi
